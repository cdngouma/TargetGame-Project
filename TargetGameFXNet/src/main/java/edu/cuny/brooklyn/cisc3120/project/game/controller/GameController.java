...
public class GameController {
    ...
    @FXML
    void newGame(ActionEvent event) throws FileNotFoundException, IOException {
    ...
    private void startNewGame() {   
    	targetGame.setGameStateChanged(false);
    	clearTargetPane();
   ...
   //     gc.drawImage(new Image("img/target.png"), xPos, yPos, cellWidth, cellHeight); If we want an image as target
    ...
    private void processShotAction(TargetGame gameState, Shot shot) {
    	...
    	gameState.setGameStateChanged(true);
    	
        // Updates stats
    	gameState.getGameStatistics().decreementNumAttempts();
        gameState.getGameStatistics().setNumOfShotsFired();
    	gameStatTableView.setItems( gameState.getGameStatistics().toObservableList()); // Updates statistics table (view)
    	
        if (gameState.getTarget().isTargetShot(shot)) {
        	 gameState.getGameStatistics().setNumOfTargetsShot();
        	 gameState.getGameStatistics().setNumOfRoundsWon();
            ...
            clearTarget();
            clearTargetPane();
            gameState.getGameStatistics().updateAccuracy();
            gameState.getGameStatistics().resetNumAttempts();
            addTarget( gameState, targetCanvas);
        }
        else if ( gameState.getGameStatistics().getNumAttempts() == 0) {
        	Alert alert = new Alert(AlertType.INFORMATION
